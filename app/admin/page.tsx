  ...
  return (
    <div className="space-y-8">
      <header className="max-w-4xl space-y-4">
        ...
        <h2 className="text-4xl font-black uppercase tracking-[0.1em] text-white md:text-6xl">Content control center</h2>
        <p className="text-sm leading-7 text-zinc-400">Manage events, routes, and media from a protected interface backed by Supabase and Cloudinary.</p>
      </header>

      <div className="grid gap-4 md:grid-cols-3 xl:max-w-4xl">
        <div className="rounded-3xl border border-zinc-800 bg-white/5 p-5">
          <p className="text-[10px] uppercase tracking-[0.4em] text-zinc-500">Events</p>
          <p className="mt-3 text-3xl font-black text-white">{events.length}</p>
        </div>
        <div className="rounded-3xl border border-zinc-800 bg-white/5 p-5">
          <p className="text-[10px] uppercase tracking-[0.4em] text-zinc-500">Routes</p>
          <p className="mt-3 text-3xl font-black text-white">{routes.length}</p>
        </div>
        <div className="rounded-3xl border border-zinc-800 bg-white/5 p-5">
          <p className="text-[10px] uppercase tracking-[0.4em] text-zinc-500">Media</p>
