    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-4 rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(244,63,94,0.12),transparent_32%),rgba(9,9,11,0.94)] p-5 sm:flex-row sm:items-end sm:justify-between sm:p-7">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-zinc-500">Prepara en escritorio y abre Modo coche para reconocimiento, dictado GPS y copiloto experimental sobre una versión guardada.</p>
        </div>
      ...
      {!storageReady ? <div className="rounded-2xl border border-amber-400/20 bg-amber-400/[0.07] p-4 text-sm text-amber-100">La biblioteca está preparada en código pero falta aplicar la migración de base de datos de RecceMind.</div> : null}
      {error ? <div className="rounded-2xl border border-red-400/20 bg-red-400/10 p-4 text-sm text-red-100">{error}</div> : null}

      {loading ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{[0, 1, 2].map((item) => <div key={item} className="h-52 animate-pulse rounded-[1.5rem] border border-zinc-800 bg-zinc-900/50" />)}</div>
      ) : stages.length ? (
            ...
            <article key={stage.id} className="group flex min-h-56 flex-col justify-between rounded-[1.5rem] border border-zinc-800 bg-zinc-950/80 p-5 transition hover:border-white/20">
              <div>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0"><p className="text-[9px] uppercase tracking-[0.2em] text-zinc-600">Actualizado {dateLabel(stage.updatedAt)}</p><h2 className="mt-2 truncate text-xl font-semibold text-white">{stage.name}</h2><p className="mt-1 truncate text-xs text-zinc-600">{stage.driverId}</p></div>
                ...
                <div className="mt-5 grid grid-cols-3 gap-2"><Metric label="Km" value={stage.distanceMeters ? (stage.distanceMeters / 1000).toFixed(2) : "—"} /><Metric label="Curvas" value={String(stage.curveCount)} /><Metric label="Notas" value={String(stage.noteCount)} /></div>
              </div>

              <div className="mt-5 grid grid-cols-[1fr_1fr_auto] gap-2">
                <Link href={`/reccemind?stage=${encodeURIComponent(stage.id)}`} className="rounded-xl bg-white px-3 py-2.5 text-center text-[9px] font-semibold uppercase tracking-[0.14em] text-black">Editar</Link>
                <Link href={`/reccemind/coche?stage=${encodeURIComponent(stage.id)}`} className="rounded-xl bg-orange-300 px-3 py-2.5 text-center text-[9px] font-semibold uppercase tracking-[0.14em] text-black">Modo coche</Link>
                <button type="button" onClick={() => void removeStage(stage)} className="rounded-xl border border-zinc-800 px-3 py-2.5 text-[9px] uppercase tracking-[0.14em] text-zinc-600 transition hover:border-red-400/30 hover:text-red-300">×</button>
              </div>
            </article>
          ))}
        </div>
      ) : storageReady ? (
        <div className="flex min-h-80 items-center justify-center rounded-[2rem] border border-dashed border-zinc-800 bg-white/[0.02] p-8 text-center"><div className="max-w-md"><p className="text-[10px] uppercase tracking-[0.3em] text-zinc-600">Biblioteca vacía</p><h2 className="mt-3 text-2xl font-semibold text-zinc-300">Guarda tu primer tramo</h2><p className="mt-3 text-sm leading-7 text-zinc-600">Analiza un KMZ, GPX, ruta o telemetría y usa “Guardar tramo” para conservar el trabajo en tu cuenta.</p><Link href="/reccemind" className="mt-5 inline-block rounded-xl border border-zinc-700 px-4 py-2.5 text-[10px] uppercase tracking-[0.18em] text-zinc-300">Ir a preparar</Link></div></div>
      ) : null}
    </div>
  );
}
