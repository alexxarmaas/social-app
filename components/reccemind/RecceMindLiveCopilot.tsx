...
export default function RecceMindLiveCopilot({ result, coordinates }: { result: RecceMindAnalysis; coordinates: RecceMindCoordinate[] }) {
      ...
      (position) => {
        ...
        if (!initializedRef.current) {
          for (const call of calls) {
            if (call.note.distance < matched.routeDistance - 30) spokenRef.current.add(call.id);
          }
          initializedRef.current = true;
        }

        if (position.coords.accuracy > MAX_GPS_ACCURACY_M) {
          setStatus("low_accuracy");
        } else if (matched.offRouteMeters > MAX_OFF_ROUTE_M) {
          setStatus("off_route");
        } else if (reverseSamplesRef.current >= 2) {
          setStatus("reverse");
        } else {
          setStatus("ready");
        }
  ...
  const requestFullscreen = async () => {
    try {
      ...
      // Fullscreen is a convenience feature; GPS mode continues without it.
    }
  };
    ...
    <section className="space-y-4 rounded-[2rem] border border-orange-400/20 bg-[radial-gradient(circle_at_top_right,rgba(249,115,22,0.14),transparent_34%),rgba(9,9,11,0.96)] p-4 sm:p-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(20rem,0.65fr)]">
        <RecceMindMap
          ...
          playhead={projection?.coordinate ?? rawPosition}
          followPlayhead={active}
        />
