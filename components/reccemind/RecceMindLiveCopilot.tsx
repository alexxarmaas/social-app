...
function buildCalls(result: RecceMindAnalysis) {
  return result.pacenotes.flatMap((note, index) => {
    ...
    return [{ id: `${index}-${note.distance}-${note.text}`, note, phrase: link ? `${note.text}, ${link}` : note.text, link }];
  });
}
...
export default function RecceMindLiveCopilot({ result, coordinates }: { result: RecceMindAnalysis; coordinates: RecceMindCoordinate[] }) {
  ...
  const [spokenVersion, setSpokenVersion] = useState(0);
  ...
  const lastSpokenPhraseRef = useRef<string | null>(null);
  const wakeLockRef = useRef<WakeLockSentinelLike | null>(null);

  const progress = projection?.routeDistance ?? 0;
  const callAhead = clampCallAheadMeters(speedMps, leadSeconds);
  void spokenVersion;
  const upcoming = calls.find((call) => !spokenRef.current.has(call.id) && call.note.distance >= progress - 20) ?? null;
  ...
  const releaseWakeLock = async () => {
    ...
    setWakeLockActive(false);
    if (current) {
      try { await current.release(); } catch { /* optional browser capability */ }
    }
  };
  ...
  useEffect(() => {
    ...
    if (distanceToCall > callAhead || spokenRef.current.has(upcoming.id)) return;
    spokenRef.current.add(upcoming.id);
    setSpokenVersion((current) => current + 1);
    speak(upcoming.phrase);
  }, [active, callAhead, distanceToCall, status, upcoming, voiceEnabled]);
  ...
  const start = () => {
    ...
    spokenRef.current = new Set<string>();
    lastSpokenPhraseRef.current = null;
    initializedRef.current = false;
    previousSampleRef.current = null;
    reverseSamplesRef.current = 0;
    setActive(true);
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        const browserSpeed = typeof position.coords.speed === "number" && Number.isFinite(position.coords.speed) && position.coords.speed >= 0 ? position.coords.speed : null;
        setSpeedMps(browserSpeed ?? derivedSpeed);

        if (!initializedRef.current) {
          for (const call of calls) if (call.note.distance < matched.routeDistance - 30) spokenRef.current.add(call.id);
  ...
  const rearmFromHere = () => {
    spokenRef.current = new Set(calls.filter((call) => call.note.distance < progress - 20).map((call) => call.id));
    setSpokenVersion((current) => current + 1);
    window.speechSynthesis?.cancel();
  };
  ...
  const repeatLast = () => {
    if (lastSpokenPhraseRef.current) speak(lastSpokenPhraseRef.current);
  };
  ...
  return (
    <section className="space-y-4 rounded-[2rem] border border-orange-400/20 bg-[radial-gradient(circle_at_top_right,rgba(249,115,22,0.14),transparent_34%),rgba(9,9,11,0.96)] p-4 sm:p-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <div className="flex items-center gap-2"><p className="text-[10px] uppercase tracking-[0.4em] text-orange-300/70">Copiloto GPS beta</p><span className={`h-2 w-2 rounded-full ${status === "ready" ? "bg-emerald-300" : active ? "bg-amber-300" : "bg-zinc-700"}`} />{wakeLockActive ? <span className="text-[8px] uppercase tracking-[0.14em] text-emerald-300/60">pantalla activa</span> : null}</div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(20rem,0.65fr)]">
        ...
        <div className="space-y-4 rounded-[1.5rem] border border-white/10 bg-black/30 p-5">
          <div className="flex items-center justify-between gap-3"><span className={`rounded-full border px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.14em] ${status === "ready" ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-100" : status === "reverse" || status === "off_route" ? "border-red-400/25 bg-red-400/10 text-red-100" : "border-amber-400/25 bg-amber-400/10 text-amber-100"}`}>{statusLabel(status)}</span><span className="font-mono text-xs text-zinc-500">{(progress / 1000).toFixed(3)} km</span></div>

          <div className="min-h-40"><div className="flex items-center justify-between gap-3"><p className="text-[9px] uppercase tracking-[0.22em] text-zinc-600">Próxima llamada</p><span className="font-mono text-sm font-bold text-orange-300">{upcoming ? `${Math.round(distanceToCall)} m` : "META"}</span></div><p className="mt-4 text-balance text-3xl font-semibold uppercase leading-tight text-white">{upcoming?.note.text ?? "META"}</p>{upcoming?.link ? <p className="mt-2 text-4xl font-black text-orange-300">{upcoming.link}</p> : null}</div>

          <div className="grid grid-cols-3 gap-2"><Metric label="Velocidad" value={speedMps !== null ? `${Math.round(speedMps * 3.6)} km/h` : "—"} /><Metric label="GPS ±" value={accuracy !== null ? `${Math.round(accuracy)} m` : "—"} /><Metric label="Eje" value={projection ? `${Math.round(projection.offRouteMeters)} m` : "—"} /></div>

          <div className="grid grid-cols-2 gap-2"><button type="button" disabled={!lastSpokenPhraseRef.current} onClick={repeatLast} className="rounded-xl border border-zinc-700 px-3 py-2.5 text-[9px] font-semibold uppercase tracking-[0.13em] text-zinc-300 disabled:opacity-25">Repetir última</button><button type="button" disabled={!upcoming} onClick={skipUpcoming} className="rounded-xl border border-zinc-700 px-3 py-2.5 text-[9px] font-semibold uppercase tracking-[0.13em] text-zinc-300 disabled:opacity-25">Saltar siguiente</button></div>

          <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-[8px] uppercase tracking-[0.18em] text-zinc-600">Anticipación</p><p className="mt-1 text-xs text-zinc-400">{leadSeconds.toFixed(1)} s · ahora ≈ {callAhead} m</p></div><select value={leadSeconds} onChange={(event) => setLeadSeconds(Number(event.target.value))} className="rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-xs text-white"><option value={3.5}>3.5 s</option><option value={4.5}>4.5 s</option><option value={5.5}>5.5 s</option><option value={6.5}>6.5 s</option></select></div></div>

          <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3"><p className="text-[8px] uppercase tracking-[0.18em] text-zinc-600">Después</p><p className="mt-1 text-sm font-medium text-zinc-300">{after?.note.text ?? "Meta"}</p></div>
